...
public class Week1 {
	...
	//This method solves the problem in this week's optional programming assignment;
	//To decode a ciphertext, given 10 ciphertexts using the same "One-Time Pad"
	//The hint is to not what happens when a letter is Xor'd with a space (the answer is,
	//it just changes to the case of the letter)
	//The other thing to note is that Xor'ing two ciphertexts together with the same pad,
	//results in an XOr of the ciphertexts
	public static void multiPad()
	{
		//Here are the ciphertexts we were given
		//Our goal is to decrypt the last one
		...
		{
			cipherBytes[i] = BitBlock.valueOfHexString(cipherTexts[i]).toByteArray();
		}
		...
		{
			if (cipherTexts[i].length() > keyLength)
			{
